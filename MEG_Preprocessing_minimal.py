...
@mem.cache
def read_raw_filtered_cached(fif_filepath, highpass=0.1):
    raw = mne.io.read_raw_fif(fif_filepath, preload=True, verbose='INFO')
    raw.filter(highpass, None, n_jobs=-1)
...
for participant in par_numbers:
    ...
    try:
        ...
        warnings.warn(f"There have been problems with reading the raw data in the fif file of Participant number {participant}. Proceeding with next participant.")
        #print(f"There have been problems with reading the raw data in the fif file of Participant number {participant}. Proceeding with next participant.")
        continue
    ...
    #--------------------- save epochs ---------------------------------------
    print('###  saving epochs')
    filename_epoch = f'{participant=}_{event_id_selection=}_{tmin=}_{tmax=}_minimalPreprocessing'
