...
for p, participant in enumerate(participants):  # (6, 7)]: # for testing purposes we might use only 1 participant, so 2 instead of 36
    ...
    #selected_func = {'mean', 'ptp_amp', 'std'}
    #x_new = extract_features(data_x, epochs.info['sfreq'], selected_func)
    ...
    window_msec = 100 # milliseconds
    window_length = window_msec * epochs.info['sfreq']

    for t in tqdm_loop:
        datapoints = data_x[:, :, t:(t+window_msec)]


    res = Parallel(-1)(delayed(run_cv)(pipe, data_x[:, :, t], gif_pos, n_splits=n_splits) for t in tqdm_loop)
    ...
    try:
        ...
        #res = Parallel(-1)(delayed(run_cv)(pipe, x_new[:, :, t], gif_pos, n_splits=n_splits) for t in tqdm_loop)
    except:
        #print(f'\n')
        warnings.warn(
            f"\n There was an error with participant number {participant}. Maybe there were too few epochs for cross validaton.")
        continue
...
plot_filename = os.path.join(plot_folderpath, f"quadrant_decoding_{classifier}_{event_id_selection=}_{tmin=}_{tmax=}_feature_extraction.png")
