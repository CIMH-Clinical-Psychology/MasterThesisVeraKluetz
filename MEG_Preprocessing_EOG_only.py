...
def autoreject_fit_cached(epochs):
    ...
    # else compute it
    ar = autoreject.AutoReject(n_jobs=-1, verbose=False, picks='eog',
                               random_state=99)
...
for participant in par_numbers:
    ...
    epochs_eog = epochs_orig.copy().pick('eog') # create copy that only contains EOG data

    assert len(epochs_eog)==144, f'sanity check failed, more or less than 144 epochs {len(epochs_eog)=}'
    print(f"Bad channels: {epochs_eog.info['bads']}")
    ...
    #ar = autoreject_fit_cached(epochs_eog)
    ...
    #epochs_eog = ar.transform(epochs_eog)
    ...
    #epochs_eog, ica = fit_apply_ica_cached(epochs_orig, epochs_eog, ica_def)

    # check ICA solution
    #explained_var_ratio = ica.get_explained_variance_ratio(epochs_eog)
    ...
    #--------------------- save epochs ---------------------------------------
    print('###  saving epochs')
    filename_epoch = f'{participant=}_{event_id_selection=}_{tmin=}_{tmax=}_EOG-only'
    filename_epoch = valid_filename(filename_epoch)
    epoch_file_path = os.path.join(epochs_folderpath, f"{filename_epoch}-epo.fif")
    epochs_eog.save(epoch_file_path, fmt='double', overwrite=True)

    #df = epochs_eog.to_data_frame(index=["condition", "epoch", "time"])
