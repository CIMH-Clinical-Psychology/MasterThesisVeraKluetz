...
def fit_apply_ica_cached(mne_obj, ica_def):
    ...
    raw_ica = mne_obj.copy().filter(1, None, n_jobs=-1)  # should be quite fast
    ...
    ica.apply(mne_obj, exclude=remove_components)
    return mne_obj, ica
...
def autoreject_fit_cached(epochs):
    ...
    # else compute it
    ar = autoreject.AutoReject(n_jobs=-1, verbose=False, picks='mag',
                               random_state=99)
    ar.fit(epochs.copy().pick('mag'))  # weirdly enough need to pick here again
