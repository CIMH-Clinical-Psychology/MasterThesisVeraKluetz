...
def valid_filename(string):
    ...
    WARNING: only call this on filenames, not on filepaths containing '/' or '\\'
    """
    ...
    allowed = ['_', '-', '.']
    filtered = [x if (x.isalnum() or x in allowed) else '_'  for x in string]
    return ''.join(filtered)

@mem.cache
def fit_apply_ica_cached(raw, ica_def):
    """contains all ICA calculations,
    expects a raw or epochs item
    returns the same object but with EOG and ECG components removed"""
    raw = raw.copy()  # work on a copy of the data, as some operations are in place
    ...
    raw_ica = raw.copy().filter(1, None, n_jobs=-1)  # should be quite fast
    ica.fit(raw_ica, picks='meg')
    idx_ecg, scores_ecg = ica.find_bads_ecg(raw, ch_name='BIO001')
    idx_eog, scores_eog = ica.find_bads_eog(raw)
    remove_components = idx_ecg + idx_eog

    # apply ICA to the raw object in place
    ica.apply(raw, exclude=remove_components)
    return raw, ica
...
for participant in par_numbers:
    ...
    fif_filepath = settings.folderdir + f"ERP-{participant}/ERP{participant}_initial_tsss_mc.fif"
    ...
    epochs = mne.Epochs(raw, events=events, event_id=event_id_selection,
                        tmin=tmin, tmax=tmax, preload=True,
                        baseline=None)  # don't subtract baseline

    assert len(epochs)==144, f'sanity check failed, more or less than 144 epochs {len(epochs)=}'
    ...
    ar = autoreject_fit_cached(epochs)
    ...
    epochs, ica = fit_apply_ica_cached(epochs, ica_def)
